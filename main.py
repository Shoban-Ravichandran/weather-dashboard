    ...
    with placeholder.container():
        while True:
            ...
            if weather_data:
                ...
                # Add Map Location using Folium
                geolocator = Nominatim(user_agent="weather_app")
                location = geolocator.geocode(city)
                if location:
                    map_center = [location.latitude, location.longitude]
                    weather_map = folium.Map(location=map_center, zoom_start=12)
                    folium.Marker([location.latitude, location.longitude], popup=city).add_to(weather_map)
                    st.subheader("City Location")
                    st.markdown("### Map of the City")
                    st.write(weather_map)
